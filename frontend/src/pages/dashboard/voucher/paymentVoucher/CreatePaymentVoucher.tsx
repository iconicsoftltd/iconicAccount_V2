...
const CreatePaymentVoucher = () => {
  ...
  const {
    ...
    defaultValues: {
      voucherNo: generateVoucherNo("PV", selectedBranch ? JSON.parse(selectedBranch)?.name : ""),
      type: "PAYMENT",
  ...
  const accountCurrentBalance = accounts?.data?.find(
    (account: any) => account?.id === watch("entries")?.[0]?.particularId
  );
  const customerCurrentBalance = customers?.data?.find(
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      if (result?.success) {
        ...
        navigate("/accounts-admin-portal/payment-voucher");
      }
    } catch (error: any) {
    ...
    >
      ...
      <div className="flex justify-end gap-3 mt-6">
        <Button
          ...
          onClick={() => navigate("/accounts-admin-portal/payment-voucher")}
        >
          Cancel
        </Button>
        <Button
