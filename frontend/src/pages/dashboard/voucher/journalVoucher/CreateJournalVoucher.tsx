  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      if (result?.success) {
        ...
        navigate("/accounts-admin-portal/journal-voucher");
      }
    } catch (error: any) {
    ...
    >
      ...
      <div className="overflow-hidden border rounded-md">
        ...
        {fields.map((field, index) => {
          ...
          const currentAccount = accounts?.data?.find((acc: any) => acc.value === selectedId);
                  ...
                  render={({ field }) => (
                    ...
                    >
                      ...
                      <SelectContent>
                        ...
                        {accountsLoading && <p className="p-2 text-xs text-gray-500">Searching...</p>}
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    <Input
                      ...
                      placeholder="0"
                      value={watch(`entries.${index}.type`) === "Debit" ? field.value || "" : ""}
                      onChange={(e) => {
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    <Input
                      ...
                      placeholder="0"
                      value={watch(`entries.${index}.type`) === "Credit" ? field.value || "" : ""}
                      onChange={(e) => {
                        const val = e.target.value.replace(/^0+(?=\d)/, "");
                        field.onChange(Number(val) || 0);
                      }}
              ...
              <div className="col-span-1 flex justify-center gap-1">
                {index === fields.length - 1 && (
                  <Button
                    size="icon"
                    type="button"
                    onClick={() => append({ particularId: 0, type: "Debit", amount: 0 })}
                  >
        ...
        <div className="grid grid-cols-12 gap-2 items-center border-t p-2 font-semibold text-gray-700 bg-gray-50">
          ...
          <div className="col-span-2">
            <Input readOnly value={totalDebit} className="text-center bg-white h-8" />
          </div>
          <div className="col-span-2">
            <Input readOnly value={totalCredit} className="text-center bg-white h-8" />
          </div>
      ...
      <div className="flex justify-end gap-3 mt-8">
        <Button
          type="button"
          variant="outline"
          onClick={() => navigate("/accounts-admin-portal/journal-voucher")}
        >
          Cancel
        </Button>
        <Button
          disabled={createVoucherLoading || totalDebit !== totalCredit || totalDebit === 0}
          type="submit"
          className="min-w-[120px]"
        >
          {createVoucherLoading ? <ButtonLoader /> : <><GrNotes className="mr-2" /> Submit</>}
        </Button>
      </div>
    </form>
  );
