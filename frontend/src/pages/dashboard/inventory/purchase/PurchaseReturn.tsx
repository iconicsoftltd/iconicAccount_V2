...
export default function PurchaseReturn() {
  ...
  const [selectedReturn, setSelectedReturn] = useState<PurchaseReturnType | null>(null);
  ...
  const purchaseReturnColumns: ColumnDef<any>[] = [
    ...
    { accessorKey: "totalAmount",
       header: "Total Amount",
       cell: ({row}) => `${currentCurrency.name} ${row.original.totalAmount.toFixed(2)}`
       },
    { accessorKey: "totalPaymentAmount", 
      header: "Total Payment Amount",
        cell: ({row}) => `${currentCurrency.name} ${row.original.totalPaymentAmount.toFixed(2)}`
    },
    {
      accessorKey: "date",
      header: "Date",
        ...
        return (
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              onClick={() =>
                navigate(
                  `/accounts-admin-portal/create-purchase-return?id=${item.id}`
                )
              }
            >
              <FaRegEdit />
    ...
    <>
      <section className="min-h-screen space-y-4 p-4">
        ...
        <ReusableTableHeader
          ...
          createButtonLink="/accounts-admin-portal/create-purchase-return"
