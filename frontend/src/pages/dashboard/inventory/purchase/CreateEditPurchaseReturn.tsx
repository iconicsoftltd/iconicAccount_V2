...
export default function PurchaseReturnForm() {
  ...
  const [challanNo, setChallanNo] = useState<string>(generateVoucherNo(
        "CHL",
  ...
  useEffect(() => {
    if (!isEditMode) {
      ...
      if (lastChallan) {
        ...
        setChallanNo(generateVoucherNo(
        "CHL",
        selectedBranch ? JSON.parse(selectedBranch)?.name : ""
      ));
        setValue("challanNo", generateVoucherNo(
        "CHL",
        selectedBranch ? JSON.parse(selectedBranch)?.name : ""
      ));
      }
  ...
  const onSubmit = async (formData: PurchaseReturnFormValues) => {
    try {
      ...
      navigate("/accounts-admin-portal/purchase-return");
