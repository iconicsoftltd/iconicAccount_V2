...
const QuotationList: React.FC = () => {
  ...
  const [quotationToDelete, setQuotationToDelete] = useState<QuotationType | null>(null);
  ...
  const [deleteQuotation, { isLoading: isDeleting }] = useDeleteQuotationsMutation();
  ...
  const columns: ColumnDef<QuotationType>[] = [
    ...
    },
    {
      ...
      header: "Discount",
      cell: ({ row }) => `${currentCurrency.name} ${row.original.discount?.toFixed(2) || "0.00"}`,
    },
    {
      id: "actions",
      header: "Actions",
        ...
        return (
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              size="icon"
              onClick={() =>
                navigate(`/accounts-admin-portal/quotation?id=${item.id}`)
              }
            >
              <FaRegEdit />
            </Button>
  ...
  return (
    <div className="p-4 min-h-screen space-y-4">
      <ReusableTableHeader
        ...
        createButtonLink="/accounts-admin-portal/quotation"
      ...
      <Dialog open={openView} onOpenChange={setOpenView}>
        <DialogContent className="sm:max-w-[1000px] max-h-[90vh] overflow-y-auto scrollbar-hide">
          ...
          {selectedQuotationId && <ViewQuotationModal quotationId={selectedQuotationId} />}
        </DialogContent>
      </Dialog>
    </div>
  );
