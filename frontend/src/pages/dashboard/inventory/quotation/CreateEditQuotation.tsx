...
export default function CreateEditQuotation() {
  ...
  const { data: editQuotation, isLoading: isLoadingQuotation } = useGetQuotationsByIdQuery(
  ...
  const [createQuotation, { isLoading: isCreating }] = useCreateQuotationsMutation();
  const [updateQuotation, { isLoading: isUpdating }] = useUpdateQuotationsMutation();
  const isLoading = isCreating || isUpdating;

  const [invoiceNo, setInvoiceNo] = useState<string>(generateVoucherNo(
        "QT",
        selectedBranch ? JSON.parse(selectedBranch)?.name : ""
      ));
...
useEffect(() => {
  if (isEditMode && editQuotation?.data) {
    ...
    setInvoiceNo(quotation.invoiceNo || generateVoucherNo(
        "QT",
        selectedBranch ? JSON.parse(selectedBranch)?.name : ""
      ));
    ...
    setValue("invoiceNo", quotation.invoiceNo || generateVoucherNo(
        "QT",
        selectedBranch ? JSON.parse(selectedBranch)?.name : ""
      ));
    ...
    if (quotation.quotationProducts && quotation.quotationProducts.length > 0) {
      const productRows: ProductRow[] = quotation.quotationProducts.map((item: any) => ({
        id: item.id,
      ...
      const firstProduct = quotation.quotationProducts[0]?.productVariation?.product;
      if (firstProduct) {
  ...
  useEffect(() => {
    if (!isEditMode) {
      ...
      if (lastInvoice) {
        ...
        setInvoiceNo(generateVoucherNo(
        "QT",
        selectedBranch ? JSON.parse(selectedBranch)?.name : ""
      ));
        setValue("invoiceNo", generateVoucherNo(
        "QT",
        selectedBranch ? JSON.parse(selectedBranch)?.name : ""
      ));
      }
  ...
  const onSubmit = async (data: QuotationCreateEditSchema) => {
    try {
      ...
      navigate("/accounts-admin-portal/quotation-list");
    } catch (err: any) {
      toast.error(
        err?.data?.message ||
            ...
            <div className="relative">
              ...
              <Popover>
                <PopoverTrigger asChild>
                  ...
                  >
                    {quotationDate ? format(quotationDate, "MM-dd-yyyy") : "Pick a date"}
