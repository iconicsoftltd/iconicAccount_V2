...
export default function SalesReturnList() {
  ...
  const [selectedReturn , setSelectedReturn] = useState<SalesReturnType | null>(null);
  ...
  const [deleteSalesReturn, { isLoading: deleteLoading }] =
    useDeleteSalesReturnMutation();

  const handleDeleteConfirm = async () => {
    try {
      await deleteSalesReturn(selectedDeleteItem.id).unwrap();
  ...
  // Table columns
  const salesReturnColumns: ColumnDef<any>[] = [
    {
      id: "select",
      header: ({ table }) => (
        <input
    ...
    { accessorKey: "customer.accountType", header: "Customer" },
    { 
      accessorKey: "totalAmount",
      header: "Total Amount",
      cell: ({ row }) => `${currentCurrency.name} ${row.original.totalAmount?.toFixed(2) || '0.00'}`
    },
    { 
      accessorKey: "totalPaymentAmount", 
      header: "Total Payment Amount",
      cell: ({ row }) => `${currentCurrency.name} ${row.original.totalPaymentAmount?.toFixed(2) || '0.00'}`
    },
    {
      accessorKey: "date",
      header: "Date",
        ...
        return (
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              onClick={() =>
                navigate(
                  `/accounts-admin-portal/create-sales-return?id=${item.id}`
                )
              }
            >
              <FaRegEdit />
    ...
    <>
      <section className="min-h-screen space-y-4 p-4">
        ...
        <ReusableTableHeader
          ...
          hasCreatePermission={getPermission("Sales_Return", "create")}
          createButtonLabel="Create"
          createButtonIcon={<Plus size={20} />}
          createButtonLink="/accounts-admin-portal/create-sales-return"
          searchPlaceholder="Search by invoice no or sale ID"
        />
      ...
      <DeleteConfirmModal
        ...
        itemName={selectedDeleteItem?.invoiceNo}
        ...
        <DialogContent className="sm:max-w-[1000px] max-h-[90vh] overflow-y-auto scrollbar-hide">
          <div className="flex flex-col h-full">
            ...
            <div className="flex-1 overflow-auto bg-gray-50/50">
              <ViewSalesReturn salesReturnData={selectedReturn} />
            </div>
          </div>
        </DialogContent>
      </Dialog>
