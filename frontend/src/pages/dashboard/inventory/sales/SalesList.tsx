...
const SalesList: React.FC = () => {
  ...
  const [deleteSale, { isLoading: isDeleting }] = useDeleteSalesMutation();

  // Extract data from API response
  const sales = salesData?.data || [];
  const totalItems = salesData?.totalCount || 0;
  ...
  const columns: ColumnDef<SalesType>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => `${currentCurrency.name} ${row.original.totalPaymentAmount}`,
    },
    {
        ...
        return (
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              size="icon"
              onClick={() =>
                navigate(`/accounts-admin-portal/sales?id=${item.id}`)
              }
            >
              <FaRegEdit />
            </Button>
  ...
  return (
    <div className="p-4 min-h-screen space-y-4">
        <ReusableTableHeader
              ...
              hasCreatePermission={getPermission("Sales", "create")}
             createButtonLink="/accounts-admin-portal/sales"
      ...
      />
       ...
       <Dialog open={openView} onOpenChange={setOpenView}>
        <DialogContent className="sm:max-w-[1000px] max-h-[90vh] overflow-y-auto scrollbar-hide">
          <DialogHeader>
            <Heading>Sales Details</Heading>
          </DialogHeader>
          {selectedSaleId && <ViewSales saleId={selectedSaleId} />}
        </DialogContent>
      </Dialog>
